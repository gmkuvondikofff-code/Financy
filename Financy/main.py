...
@app.post("/ask-ai")
async def ask_ai(query: str = Form(...), email: str = Form(...), db: Session = Depends(get_db)):
    products = db.query(models.Product).filter(models.Product.user_email == email).all()
    debts = db.query(models.Debt).filter(models.Debt.user_email == email).all()
    
    # Kontekst tayyorlash
    context = "Tovarlar: " + ", ".join([f"{p.name}" for p in products])
    context += " | Jami qarzlar: " + str(sum(d.remaining_amount for d in debts)) + " UZS"
    
    # ENDI BU YERDA XATO CHIQMAYDI
